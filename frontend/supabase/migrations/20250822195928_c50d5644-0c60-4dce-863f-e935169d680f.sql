-- Remover as views que falharam e criar funções que respeitam RLS
DROP VIEW IF EXISTS dashboard_kpis;
DROP VIEW IF EXISTS dashboard_cost_evolution;
DROP VIEW IF EXISTS dashboard_cost_distribution;
DROP VIEW IF EXISTS dashboard_billing_periods;
DROP VIEW IF EXISTS dashboard_organization_details;
DROP VIEW IF EXISTS dashboard_available_cycles;

-- Função para KPIs principais (agregados)
...
-- Função para evolução de custos
...
-- Função para distribuição de custos
...
-- Função para métricas por ciclo
...
-- Função para detalhes de organizações (incluindo sandbox)
...
-- Função para ciclos disponíveis
